...
The docker-specific deny list lives in DENYLIST.md in this skill's folder;
its first line points back at the base ollama-shell list, which still applies
to every command string. Read it before reviewing any non-read-only draft.
...
4. **Command intent:** `python "$SCRIPT" draft-command "<task, with the real names>" --shell bash|powershell`.
   ...
   If the script printed classification: read-only, run the draft without
   review. Any other draft: Read both DENYLIST.md files first (the additions
